...
void qdp_vaxpy_chp( char *);
void qdp_vaxpy_chm( char *);
...
/* qdp_vaxpy_chp(y, a, x, y, n_4vec)
 *
 * Operation: z = a x +/- (1/2)(1 + g5) y
 *
 ...
 *        z[i][spin][col] = a * x[i][spin][col] +/- y[i][spin][col];
 *     }
 *   }
 *   for(spin=2; spin < Nspin; spin++) {
 *     for(col=0; col < Ncol; col++) { 
 *       z[i][spin][col] = a * x[i][spin][col]
 *     }
 *   }
 * }
 */

void qdp_vaxpy_chp( char *name)
{
  ...
  for( rei=0; rei<2; rei++) {
    if( !do_minus ) { 
      queue_fmadd(z[co][rei], a, x[co][rei], y[co][rei]);
    }
    else {
      make_inst(FMACPIPE,FMSUB, z[co][rei], a, x[co][rei], y[co][rei]);
    }
  ...
  co=0;
  for(rei=0; rei<2;rei++) {
    queue_fmul(z[co][rei], a, x[co][rei]);
    queue_fstore(z[co][rei], VEC_IMM[co][rei], outptr);
  }
  co=1;
  for(rei=0; rei<2; rei++) { 
...
/* qdp_vadd_chm(z, a, x, y, n_4vec)
 *
 * Operation: y = a *x +  (1/2)(1 - g5) x
 * 
 *  g5 is in chiral basis diag(1,1,-1, -1)
