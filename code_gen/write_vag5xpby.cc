...
/* qdp_vag5xpby(y, a, x, b, y, n_4vec)
 *
 * Operation: z = a g5 x +/- b y
 *
...
void qdp_vag5xpby( char *name)
{
  ...
  for( rei=0; rei<2; rei++) {
    queue_fmul(ftmp, a, x[co][rei]);
    queue_fneg(ftmp2,ftmp);
    if( !do_minus ) { 
      queue_fmadd(z[co][rei], b, y[co][rei], ftmp2);
    }
    else {
      queue_fnmsub( z[co][rei], b, y[co][rei], ftmp2);
    }
    queue_fstore(z[co][rei], VEC_IMM[co][rei], outptr);
  }
