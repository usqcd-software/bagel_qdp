...
void qdp_vadd_chp( char *);
void qdp_vadd_chm( char *);
...
/* qdp_vadd_chp(y, x, y, n_4vec)
 *
 * Operation: z = x +/- (1/2)(1 + g5) y
 *
 ...
 *        z[i][spin][col] = y[i][spin][col] +/- x[i][spin][col];
 *     }
 *   }
 *   for(spin=2; spin < Nspin; spin++) {
 *     for(col=0; col < Ncol; col++) { 
 *       z[i][spin][col] = y[i][spin][col]
 *     }
 *   }
 * }
 */

void qdp_vadd_chp( char *name)
{
  ...
  for( rei=0; rei<2; rei++) {
    if( !do_minus ) { 
      queue_fadd(z[co][rei], x[co][rei], y[co][rei]);
    }
    else {
      queue_fsub(z[co][rei], x[co][rei], y[co][rei]);
    }
