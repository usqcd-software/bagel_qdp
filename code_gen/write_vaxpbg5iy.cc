 ...
 *        z[i][spin][col][re] = a * x[i][spin][col][re] 
 *                                         -/+ b * i * y[i][spin][col][im];
 *        z[i][spin][col][im] = a * x[i][spin][col][im] 
 *                                         +/- b * i * y[i][spin][col][re];
 *     }
 *   }
...
void qdp_vaxpbg5iy( char *name)
{
  ...
  co = 0;
  queue_fmul(ftmp, a, x[co][RE]);
 
  if( !do_minus ) { 
    queue_fnmsub(z[co][RE], b, y[co][IM], ftmp);
  }
  else {
    queue_fmadd( z[co][RE], b, y[co][IM], ftmp);
  }
  queue_fstore(z[co][RE], VEC_IMM[co][RE], outptr);


  queue_fmul(ftmp, a, x[co][IM]);
  if( !do_minus ) { 
    queue_fmadd(z[co][IM], b, y[co][RE], ftmp);
  }
  else {
    queue_fnmsub( z[co][IM], b, y[co][RE], ftmp);
  }
